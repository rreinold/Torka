...
export default function Reader() {
  ...
  const [submissionResponse, setSubmissionResponse] = useState<Record<string, any> | null>(null);
  ...
  const currentBook = books[selectedBookId] || books.economics;
  const totalPages = currentBook.sections.length;
  ...
  useEffect(() => {
    ...
    setFormatUsage(new Map());
    setQuizScores(new Map());
    setSearchQuery("");
    setSearchMatches([]);
    setCurrentSearchResult(0);
    autoGeneratedSectionsRef.current.clear();
    requestedRecommendationsRef.current.clear();
  }, [selectedBookId]);
  ...
  const handleMediaAdd = useCallback(async (sectionId: number, type: "image" | "audio") => {
    const section = currentBook.sections.find((s) => s.id === sectionId);
    if (!section) {
      return;
    }
  ...
  const handleQuizSubmit = useCallback(async (sectionId: number, score: number) => {
    ...
    try {
      ...
      // Store the submission response for the radar chart
  ...
  useEffect(() => {
    const section = currentBook.sections[currentPage - 1];
    if (!section) {
      return;
    }
      ...
      (async () => {
        try {
          ...
          const metadata = {
            ...
            contentType: "expository" as const,
            readingLevel: "college" as const,
          };
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {submissionResponse && (
          <div className="absolute bottom-4 right-4 z-10">
            <SubmissionRadarChart data={submissionResponse} />
          </div>
        )}
      </div>
    </div>
